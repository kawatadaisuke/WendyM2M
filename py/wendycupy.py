...
def _nbody_force(N, x, m, mtot, omega2):
    ...
    x_gpu = cupy.array(x)
    m_gpu = cupy.array(m)
    mtot_gpu = cupy.array(mtot)
    indx_gpu = cupy.argsort(x_gpu)
    mord_gpu = m_gpu[indx_gpu]
    cumulmass_gpu = cupy.zeros(N)
    cumulmass_gpu[indx_gpu] = cupy.cumsum(mord_gpu)
    revcumulmass_gpu = mtot_gpu-cumulmass_gpu
    cumulmass_gpu -= m_gpu
    # for ii in range(10):
    ...
    if omega2 <=0.0:
      a_gpu = revcumulmass_gpu-cumulmass_gpu
    else:
      omega2_gpu = cupy.array(omega2)
      a_gpu = revcumulmass_gpu-cumulmass_gpu-omega2_gpu*x_gpu

    a = cupy.asnumpy(a_gpu)
    return a
